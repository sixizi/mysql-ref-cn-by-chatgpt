...
下面是 **Normalization, Merging, and Autowrapping of JSON Values（JSON 值的规范化、合并和自动封装）** 的中文翻译：
...
- 从左到右扫描，如果某个键（key）重复出现，则**丢弃前面出现的同名键**，只保留最后出现的键值对。
...
- MySQL 会删除原始 JSON 文档中键、值或元素之间的多余空格。

- 在显示时，每个逗号（,）或冒号（:）后面会保留（或必要时插入）一个空格，以增强可读性。

- 所有 MySQL 生成 JSON 值的函数（见 [14.17.2 创建 JSON 值的函数](#14172-创建-json-值的函数)）都返回规范化后的值。

- 为了提高查找效率，MySQL 还会对 JSON 对象的键进行排序。

  > 键的排序结果可能会在不同版本间变化，并不保证跨版本一致。
...
> JSON_MERGE_PRESERVE() 与旧版本的 JSON_MERGE() 功能相同（8.0.3 起更名）。

> JSON_MERGE() 在 8.0 中依然作为 JSON_MERGE_PRESERVE() 的别名，但已废弃，未来版本可能移除。
...
- **JSON_MERGE_PRESERVE()**：将后面的数组拼接到第一个数组末尾
- **JSON_MERGE_PATCH()**：将每个参数视为仅包含一个元素的数组，然后采用**最后一个值获胜**的规则
...
- **JSON_MERGE_PRESERVE()**：相同键的值合并为数组，数组包含该键的所有唯一值
- **JSON_MERGE_PATCH()**：相同键的值仅保留最后一个（从左到右扫描）
...
在需要数组的上下文中，如果值不是数组，会自动用 [ 和 ] 包裹，转换成数组：



- JSON_MERGE_PRESERVE()：保留所有值
- JSON_MERGE_PATCH()：只保留最后一个值
