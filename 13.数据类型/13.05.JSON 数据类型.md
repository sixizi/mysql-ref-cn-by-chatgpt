...
**修改 JSON 的函数**（路径按顺序处理，上一个修改的结果会影响下一个）：



- **JSON_SET()**：存在则替换，不存在则添加。
- **JSON_INSERT()**：只添加，不替换。
- **JSON_REPLACE()**：只替换，忽略不存在的路径。
- **JSON_REMOVE()**：删除指定路径对应的值。
...
如果你需要，我可以帮你做一个 **MySQL JSON 路径语法 & 函数速查表**，把这些规则和例子都整理成表格+示意图，方便你开发时快速查找。
