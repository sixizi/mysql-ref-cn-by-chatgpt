...
好的，下面是你提供的 **Creating JSON Values（创建 JSON 值）** 段落的完整中文翻译，保留了 Markdown 格式和关键字标记：
...
## **创建 JSON 值（Creating JSON Values）**
...
如上述示例所示，JSON 数组和对象可以包含以下类型的标量值：
...
MySQL 还提供了多个用于生成 JSON 值的函数（见 [14.17.2 创建 JSON 值的函数](#14172-创建-json-值的函数)），

也可以使用 CAST(value AS JSON) 将其他类型的值转换为 JSON 类型（见 **JSON 与非 JSON 值之间的转换**）。
...
在 MySQL 中，JSON 值以字符串形式书写。

MySQL 会解析出现在需要 JSON 值的上下文中的任何字符串，如果该字符串不是有效 JSON，就会产生错误。
...
- 向声明为 JSON 数据类型的列插入值
- 将参数传递给需要 JSON 值的函数（在文档中通常表示为 json_doc 或 json_val）
...
-- "Invalid value." at position 6 in value '[1, 2,'
```
...
JSON 上下文中的字符串使用 utf8mb4 字符集和 utf8mb4_bin 排序规则。

其他字符集会根据需要转换为 utf8mb4。

对于 ascii 和 utf8mb3，不需要转换，因为它们是 utf8mb4 的子集。
...
但是，用户定义变量不能是 JSON 数据类型，因此即使 @j 的显示结果看起来像 JSON，

并且具有相同的字符集和排序规则，它实际上是字符串类型（赋值时由 JSON 转为字符串）。
...
双反斜杠可以防止 MySQL 执行转义序列解析，而是将字符串原样传递给存储引擎。
...
如果你愿意，我可以接着翻译 **Normalization, Merging, and Autowrapping of JSON Values**，这样你的 13.5 小节就会衔接到下一个功能点。
