...
这表明操作系统用户 `antonio` 被认证为具有 `data_entry` MySQL 用户的权限，并且代理通过默认代理账户进行。
  ...
  >
  > - 为了降低无意中使用 `mysql_clear_password` 插件的可能性，MySQL 客户端必须显式启用它（例如，通过 `--enable-cleartext-plugin` 选项）。参见 [8.4.1.4 节, “客户端明文可插拔认证”](#8.4.1.4-客户端明文可插拔认证)。
  > - 为避免在启用 `mysql_clear_password` 插件时密码暴露，MySQL 客户端应使用加密连接连接

到 MySQL 服务器。参见 [8.3.1 节, “配置 MySQL 使用加密连接”](#8.3.1-配置-mysql-使用加密连接)。
...
- 在 `/etc/group` 中创建 `shadow` 组。
- 将 `mysql` 操作系统用户添加到 `/etc/group` 中的 `shadow` 组。
- 将 `/etc/group` 分配给 `shadow` 组并启用组读权限：
  ```bash
