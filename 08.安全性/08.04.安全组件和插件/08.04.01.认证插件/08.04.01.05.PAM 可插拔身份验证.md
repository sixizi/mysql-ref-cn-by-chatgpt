...
- 程序连接时传递的外部用户名不同的 MySQL 用户名。使用认证字符串控制从外部用户名到 MySQL 用户名的映射。如果您想利用代理用户功能，认证字符串必须包含这种映射。
  ...
  通过创建一个名为 `/etc/pam.d/mysql-unix` 的 `mysql-unix` PAM 服务文件，设置 PAM 使用传统的 Unix 密码认证 MySQL 连接。文件内容依赖于系统，因此请检查 `/etc/pam.d` 目录中现有的与登录相关的文件，了解它们的外观。在 Linux 上，`mysql-unix` 文件可能如下所示：
  ...
  创建一个与操作系统用户名相同的 MySQL 账户，并定义其使用 PAM 插件和 `mysql-unix` PAM 服务进行认证：
  ...
  使用 `mysql` 命令行客户端以 `antonio` 身份连接到 MySQL 服务器。例如：
  ...
  **注意**
  ...
  - 为避免在启用 `mysql_clear_password` 插件时密码暴露，MySQL 客户端应使用加密连接连接到 MySQL 服务器。参见 [8.3.1 节, “配置 MySQL 使用加密连接”](#8.
  - 3.1-配置-mysql-使用加密连接)。
