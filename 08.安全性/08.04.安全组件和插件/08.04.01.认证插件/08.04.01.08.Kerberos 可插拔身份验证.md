...
- [Kerberos 可插拔身份验证的先决条件](#prerequisites-for-kerberos-pluggable-authentication)
- [MySQL 用户的 Kerberos 身份验证工作原理](#how-kerberos-authentication-of-mysql-users-works)
- [安装 Kerberos 可插拔身份验证](#installing-kerberos-pluggable-authentication)
- [使用 Kerberos 可插拔身份验证](#using-kerberos-pluggable-authentication)
- [Kerberos 身份验证调试](#kerberos-authentication-debugging)
...
- 在 MySQL 服务器主机上，服务器端服务密钥表文件用于验证 MySQL 服务器从客户端接收

的服务票证。密钥表文件名通过 `authentication_kerberos_service_key_tab` 系统变量进行配置。
...
- [验证 Kerberos 可用性](#verify-kerberos-availability)
- [创建使用 Kerberos 身份验证的 MySQL 帐户](#create-a-mysql-account-that-uses-kerberos-authentication)
- [使用 MySQL 帐户连接到 MySQL 服务器](#use-the-mysql-account-to-connect-to-the-mysql-server)
- [Kerberos 身份验证的客户端配置参数](#client-configuration-parameters-for-kerberos-authentication)
...
- Kerberos 管理员应将用户名注册为 Kerberos 主



体。此名称包括一个领域名。客户端使用主体名称和密码进行 Kerberos 身份验证并获取票证授予票证 (TGT)。
...
- 如果 Kerberos 缓存中没有 TGT，客户端 Kerberos 身份验证插件本身可以从 KDC 获取 TGT。使用 MySQL 用户名和密码选项调用客户端，然后在提示时输入 UPN 密码：
    ...
    客户端 Kerberos 身份验证插件使用操作系统登录名作为 MySQL 用户名。它组合该用户名和用户 MySQL 帐户中的领域以构建 UPN。客户端插件使用 U
...
PN 和密码获取 TGT，使用 TGT 获取 MySQL ST，并使用 ST 进行身份验证到 MySQL 服务器。
...
在客户端主机上，可以使用客户端密钥表文件在不提供密码的情况下
...
获取 TGT 和 TS。有关密钥表文件的信息，请参见 https://web.mit.edu/kerberos/krb5-latest/doc/basic/keytab_def.html。
