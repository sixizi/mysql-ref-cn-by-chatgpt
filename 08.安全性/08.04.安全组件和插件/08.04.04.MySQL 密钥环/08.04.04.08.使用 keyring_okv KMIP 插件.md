...
.0.29 开始，您可以指定多个备用服务器（最多 64 个）。如果您这样做，`keyring_okv` 插件会遍历它们，直到它可以建立连接，如果不能，则失败。要添加额外的备用服务器，请编辑 `okvclient.ora` 文件，以逗号分隔的列表格式指定服务器的 IP 地址和端口号。例如：
...
转到 Oracle Key Vault 安装目录，并运行以下命令测试设置：
...
（可选）如果希望密码保护密钥文件，请使用[密码保护 `keyring_okv` 密钥文件](#password-protecting-the-keyring_okv-key-file)中的说明。
...
以具有证书颁发机构访问权限的管理员凭据连接到 KeySecure 管理控制台。

导航到 Security >> Local CAs 并创建一个本地证书颁发机构 (CA)。

转到 Trusted CA Lists。选择 Default 并点击 Properties。然后选择 Edit for Trusted Certificate Authority List 并添加刚刚创建的 CA。

下载 CA 并将其保存到 `ssl` 目录中，文件名为 `CA.pem`。

导航到 Security >> Certificate Requests 并创建一个证书。然后，您可以下载包含证书 PEM 文件的压缩 tar 文件。

解压缩和解包下载的文件中的 PEM 文件。例如，如果文件名为 `csr_w_pk_pkcs8.gz`，使用以下命令解压缩和解包：
...
使用以下 `openssl` 命令解密私钥并创建一个名为 `key.pem` 的文件：
...
导航到 Security >> Local CAs。选择之前创建的 CA（您下载以创建 `CA.pem` 文件的 CA），然后点击 Sign Request。将剪贴板中的证书请求粘贴到相应位置，选择证书用途为 Client（密钥环是 KeySecure 的客户端），然后点击 Sign Request。结果是在新页面中生成一个由所选 CA 签名的证书。

将签名证书复制到剪贴板，然后将剪贴板内容保存为 `ssl` 目录中的 `cert.pem` 文件。
