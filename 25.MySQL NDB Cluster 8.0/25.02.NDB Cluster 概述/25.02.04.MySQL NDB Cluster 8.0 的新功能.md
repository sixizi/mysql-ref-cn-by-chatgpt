...
从 NDB 8.0.29 开始，对缺少索引的 NDB 表执行的 SQL 查询如果不使用任何缺失的索引，则会成功。否则，查询会被拒绝并显示 `ER_NOT_KEYFILE` 错误。在这种情况下，您可以使用 `ALTER TABLE ... ALTER INDEX ... INVISIBLE` 来防止 MySQL 优化器尝试使用索引，或使用适当的 SQL 语句删除索