...
- innodb_sort_buffer_size
  ...
  - 在线 DDL 操作中创建或重建二级索引的排序缓冲区大小。然而，从 MySQL 8.0.27 开始，此职责由 `innodb_ddl_buffer_size` 变量承担。
  - 在记录在线 DDL 操作期间并发 DML 时，临时日志文件的扩展量，以及临时日志文件读写缓冲区的大小。
...
- innodb_spin_wait_delay
  ...
  轮询自旋锁之间的最大延迟。此机制的低级实现因硬件和操作系统组合的不同而异，因此延迟不对应固定的时间间隔。

  可以与 `innodb_spin_wait_pause_multiplier` 变量结合使用，以更好地控制自旋锁轮询延迟的持续时间。

  有关更多信息，请参见 [第17.8.8节, “配置自旋锁轮询”](#17.8.8-配置自旋锁轮询)。
...
- innodb_spin_wait_pause_multiplier
  ...
  定义一个乘数值，用于确定线程等待获取互斥锁或读写锁时，在自旋等待循环中执行 `PAUSE` 指令的数量。
...
- innodb_stats_auto_recalc
  ...
  当表中的数据发生重大变化时，InnoDB 会自动重新计算持久统计信息。阈值为表中行数的10%。此设置适用于在启用 `innodb_stats_persistent` 选项时创建的表。自动统计信息重新计算也可以通过在 `CREATE TABLE` 或 `ALTER TABLE` 语句中指定 `STATS_AUTO_RECALC=1` 来配置。用于生成统计信息的数据量由 `innodb_stats_persistent_sample_pages` 变量控制。

  有关更多信息，请参见 [第17.8.10.1节, “配置持久优化器统计参数”](#17.8.10.1-配置持久优化器统计参数)。
...
- innodb_stats_include_delete_marked
  ...
  默认情况下，InnoDB 在计算统计信息时读取未提交的数据。如果未提交的事务从表中删除行，则 InnoDB 在计算行估计和索引统计时排除已删除标记的记录，这可能会导致并发使用非 `READ UNCOMMITTED` 事务隔离级别操作表的其他事务执行计划不佳。为了避免这种情况，可以启用 `innodb_stats_include_delete_marked` 以确保 InnoDB 在计算持久优化器统计信息时包括删除标记的记录。

  启用 `innodb_stats_include_delete_marked` 时，`ANALYZE TABLE` 在重新计算统计信息时会考虑删除标记的记录。

  `innodb_stats_include_delete_marked` 是一个影响所有 InnoDB 表的全局设置，仅适用于持久优化器统计信息。

  有关相关信息，请参见 [第17.8.10.1节, “配置持久优化器统计参数”](#17.8.10.1-配置持久优化器统计参数)。
...
- innodb_stats_method
  ...
  | **类型**             | 枚举                                                |
  | **默认值**           | `nulls_equal`                                       |
  | **有效值**           | `nulls_equal`<br>`nulls_unequal`<br>`nulls_ignored` |

  当收集 InnoDB 表的索引值分布统计信息时，服务器如何处理 `NULL` 值。允许的值为 `nulls_equal`、`nulls_unequal` 和 `nulls_ignored`。对于 `nulls_equal`，所有 `NULL` 索引值被认为相等，并形成一个大小等于 `NULL` 值数量的单一值组。对于 `nulls_unequal`，`NULL` 值被认为不相等，每个 `NULL` 形成一个大小为1的独立值组。对于 `nulls_ignored`，忽略 `NULL` 值。

  生成表统计信息的方法会影响优化器选择查询执行索引的方式，如 [第10.3.8节, “InnoDB 和 MyISAM 索引统计信息收集”](#10.3.8-innodb-和-myisam-索引统计信息收集)中所述。
...
- innodb_stats_on_metadata
  ...
  此选项仅适用于配置为非持久的优化器统计信息。当禁用 `innodb_stats_persistent` 或使用 `STATS_PERSISTENT=0` 创建或更改单个表时，优化器统计信息不会持久保存到磁盘。有关更多信息，请参见 [第17.8.10.2节, “配置非持久优化器统计参数”](#17.8.10.2-配置非持久优化器统计参数)。

  启用 `innodb_stats_on_metadata` 时，当执行元数据语句（如 `SHOW TABLE STATUS`）或访问 `Information Schema` 的 `TABLES` 或 `STATISTICS` 表时，InnoDB 会更新非持久统计信息（这些更新类似于 `ANALYZE TABLE` 的作用）。禁用时，InnoDB 在这些操作期间不会更新统计信息。禁用该设置可以提高具有大量表或索引的模式的访问速度。它还可以提高涉及 InnoDB 表的查询执行计划的稳定性。

  要更改设置，请执行语句 `SET GLOBAL innodb_stats_on_metadata=mode`，其中 `mode` 为 `ON` 或 `OFF`（或 `1` 或 `0`）。更改设置需要足够的权限来设置全局系统变量（请参见 [第7.1.9.1节, “系统变量权限”](#7.1.9.1-系统变量权限)），并立即影响所有连接的操作。
...
- innodb_stats_persistent
  ...
  指定 InnoDB 索引统计信息是否持久保存到磁盘。否则，统计信息可能会频繁重新计算，从而导致查询执行计划的变化。此设置在创建表时存储在每个表中。可以在创建表之前在全局级别设置 `innodb_stats_persistent`，或者使用 `CREATE TABLE` 和 `ALTER TABLE` 语句的 `STATS_PERSISTENT` 子句来覆盖系统范围的设置，为单个表配置持久统计信息。
...
- innodb_stats_persistent_sample_pages
  ...
  在估算索引列的基数和其他统计信息（如 `ANALYZE TABLE` 计算的统计信息）时要采样的索引页数。增加该值可以提高索引统计信息的准确性，从而改进查询执行计划，但代价是在执行 `ANALYZE TABLE` 时增加了 I/O。有关更多信息，请参见 [第17.8.10.1节, “配置持久优化器统计参数”](#17.8.10.1-配置持久优化器统计参数)。

  > **注意**
  >
  > 设置较高的 `innodb_stats_persistent_sample_pages` 值可能会导致 `ANALYZE TABLE` 执行时间过长。要估算 `ANALYZE TABLE` 访问的数据库页数，请参见 [第17.8.10.3节, “估算 InnoDB 表的 ANALYZE TABLE 复杂性”](#17.8.10.3-估算-innodb-表的-analyze-table-复杂性)。

  `innodb_stats_persistent_sample_pages` 仅在为表启用 `innodb_stats_persistent` 时适用；当 `innodb_stats_persistent` 被禁用时，适用 `innodb_stats_transient_sample_pages`。
...
- innodb_stats_transient_sample_pages
  ...
  在估算索引列的基数和其他统计信息（如 `ANALYZE TABLE` 计算的统计信息）时要采样的索引页数。默认值为8。增加该值可以提高索引统计信息的准确性，从而改进查询执行计划，但代价是在打开 InnoDB 表或重新计算统计信息时增加了 I/O。有关更多信息，请参见 [第17.8.10.2节, “配置非持久优化器统计参数”](#17.8.10.2-配置非持久优化器统计参数)。

  > **注意**
  >
  > 设置较高的 `innodb_stats_transient_sample_pages` 值可能会导致 `ANALYZE TABLE` 执行时间过长。要估算 `ANALYZE TABLE` 访问的数据库页数，请参见 [第17.8.10.3节, “估算 InnoDB 表的 ANALYZE TABLE 复杂性”](#17.8.10.3-估算-innodb-表的-analyze-table-复杂性)。

  `innodb_stats_transient_sample_pages` 仅在为表禁用 `innodb_stats_persistent` 时适用；当 `innodb_stats_persistent` 启用时，适用 `innodb_stats_persistent_sample_pages`。替代 `innodb_stats_sample_pages`。有关更多信息，请参见 [第17.8.10.2节, “配置非持久优化器统计参数”](#17.8.10.2-配置非持久优化器统计参数)。
...
- innodb_status_output
  ...
  启用或禁用标准 InnoDB 监视器的周期性输出。还可与 `innodb_status_output_locks` 结合使用，以启用或禁用 InnoDB 锁监视器的周期性输出。有关更多信息，请参见 [第17.17.2节, “启用 InnoDB 监视器”](#17.17.2-启用-innodb-监视器)。
...
- innodb_status_output_locks
  ...
  启用或禁用 InnoDB 锁监视器。启用后，InnoDB 锁监视器会在 `SHOW ENGINE INNODB STATUS` 输出和定期打印到 MySQL 错误日志中的输出中打印有关锁的额外信息。InnoDB 锁监视器的定期输出是标准 InnoDB 监视器输出的一部分。因此，必须启用标准 InnoDB 监视器，才能让 InnoDB 锁监视器定期将数据打印到 MySQL 错误日志中。有关更多信息，请参见 [第17.17.2节, “启用 InnoDB 监视器”](#17.17.2-启用-innodb-监视器)。
...
- innodb_strict_mode
  ...
  启用 `innodb_strict_mode` 后，InnoDB 在检查无效或不兼容的表选项时返回错误而不是警告。

  它检查 `KEY_BLOCK_SIZE`、`ROW_FORMAT`、`DATA DIRECTORY`、`TEMPORARY` 和 `TABLESPACE` 选项是否彼此兼容以及与其他设置是否兼容。

  启用 `innodb_strict_mode=ON` 还会在创建或修改表时启用行大小检查，以防止由于记录对于所选页面大小过大而导致 `INSERT` 或 `UPDATE` 失败。

  可以在启动 mysqld 时通过命令行启用或禁用 `innodb_strict_mode`，或在 MySQL 配置文件中启用或禁用。还可以在运行时通过语句 `SET [GLOBAL|SESSION] innodb_strict_mode=mode` 启用或禁用 `innodb_strict_mode`，其中 `mode` 为 `ON` 或 `OFF`。更改全局设置需要有足够权限设置全局系统变量（请参见 [第7.1.9.1节, “系统变量权限”](#7.1.9.1-系统变量权限)），并影响随后连接的所有客户端。任何客户端都可以更改 `innodb_strict_mode` 的会话设置，该设置仅影响该客户端。

  从 MySQL 8.0.26 开始，设置此系统变量的会话值是受限操作。会话用户必须具有足够的权限来设置受限会话变量。请参见 [第7.1.9.1节, “系统变量权限”](#7.1.9.1-系统变量权限)。
...
- innodb_sync_array_size
  ...
  定义互斥锁/锁等待数组的大小。增加该值会拆分用于协调线程的内部数据结构，以在有大量等待线程的工作负载中提高并发性。此设置必须在 MySQL 实例启动时配置，之后不能更改。对于频繁产生大量等待线程（通常超过768个线程）的工作负载，建议增加该值。
...
- innodb_sync_debug
  ...
  启用 InnoDB 存储引擎的同步调试检查。此选项仅在使用 `WITH_DEBUG` CMake 选项编译时可用。
...
- innodb_table_locks
  ...
  如果 `autocommit = 0`，InnoDB 遵守 `LOCK TABLES`；MySQL 不会从 `LOCK TABLES ... WRITE` 返回，直到所有其他线程释放了该表的所有锁。`innodb_table_locks` 的默认值为 `1`，这意味着如果 `autocommit = 0`，`LOCK TABLES` 会导致 InnoDB 在内部锁定表。

  `innodb_table_locks = 0` 对于使用 `LOCK TABLES ... WRITE` 显式锁定的表没有影响。对于通过触发器隐式（例如，通过触发器）或通过 `LOCK TABLES ... READ` 读取或写入锁定的表，它确实有影响。

  有关相关信息，请参见 [第17.7节, “InnoDB 锁定和事务模型”](#17.7-innodb-锁定和事务模型)。
...
- innodb_temp_data_file_path
  ...
  如果没有为 `innodb_temp_data_file_path` 指定值，默认行为是在 `innodb_data_home_dir` 目录中创建一个名为 `ibtmp1` 的自动扩展数据文件。初始文件大小略大于 12MB。

  全局临时表空间数据文件规范的语法包括文件名、文件大小、自动扩展和最大属性：

  ```plaintext
  file_name:file_size[:autoextend[:max:max_file_size]]
  ```

  全局临时表空间数据文件不能与另一个 InnoDB 数据文件同名。创建全局临时表空间数据文件的任何能力或错误都被视为致命错误，并且服务器启动会被拒绝。

  文件大小可以通过在大小值后附加 `K`、`M` 或 `G` 来指定为 KB、MB 或 GB。文件大小的总和必须略大于 12MB。

  单个文件的大小限制由操作系统决定。在支持大文件的操作系统上，文件大小可以超过 4GB。全局临时表空间数据文件不支持使用裸磁盘分区。

  自动扩展和最大属性只能用于 `innodb_temp_data_file_path` 设置中最后指定的数据文件。例如：
  ...
  自动扩展选项使数据文件在空间不足时自动增加大小。自动扩展增量默认是 64MB。要修改增量，请更改 `innodb_autoextend_increment` 变量设置。

  全局临时表空间数据文件的目录路径是通过连接 `innodb_data_home_dir` 和 `innodb_temp_data_file_path` 定义的路径来形成的。

  在以只读模式运行 InnoDB 之前，将 `innodb_temp_data_file_path` 设置到数据目录之外的位置。路径必须相对于数据目录。例如：
...
- innodb_temp_tablespaces_dir
  ...
  从 MySQL 8.0.16 开始，会话临时表空间总是存储用户创建的临时表和优化器使用 InnoDB 创建的内部临时表（之前，内部临时表的磁盘存储引擎由 `internal_tmp_disk_storage_engine` 系统变量决定，现在不再支持该变量。参见磁盘上的内部临时表的存储引擎）。
...
- innodb_thread_concurrency
  ...
  定义允许进入 InnoDB 的最大线程数。值为 `0`（默认值）表示无限并发（没有限制）。该变量旨在对高并发系统进行性能调优。

  InnoDB 尽量使进入 InnoDB 的线程数不超过 `innodb_thread_concurrency` 限制。等待锁的线程不计入并发执行的线程数。

  正确的设置取决于工作负载和计算环境。如果您的 MySQL 实例与其他应用程序共享 CPU 资源，或者您的工作负载或并发用户数量在增加，请考虑设置该变量。测试一系列值以确定最佳性能的设置。`innodb_thread_concurrency` 是一个动态变量，可以在活动测试系统上尝试不同的设置。如果特定设置表现不佳，可以快速将 `innodb_thread_concurrency` 重新设置为 `0`。

  以下指南可以帮助找到并保持适当的设置：

  - 如果工作负载的并发用户线程数始终较少且不影响性能，请将 `innodb_thread_concurrency` 设置为 `0`（无限制）。

  - 如果工作负载始终很重或偶尔会激增，请设置 `innodb_thread_concurrency` 值并进行调整，直到找到最佳性能的线程数。例如，假设系统通常有 40 到 50 个用户，但有时会增加到 60、70 或更多。通过测试发现，性能在限制为 80 个并发用户时仍然保持稳定。在这种情况下，将 `innodb_thread_concurrency` 设置为 80。

  - 如果不希望 InnoDB 为用户线程使用超过一定数量的虚拟 CPU（例如 20 个虚拟 CPU），请将 `innodb_thread_concurrency` 设置为该数量（或根据性能测试可能更低）。如果目标是将 MySQL 与其他应用程序隔离，可以考虑将 mysqld 进程专门绑定到虚拟 CPU。然而，如果 mysqld 进程并非始终繁忙，专属绑定可能会导致硬件使用不佳。在这种情况下，可以将 mysqld 进程绑定到虚拟 CPU，但允许其他应用程序使用部分或全部虚拟 CPU。

    > **注意**
    >
    > 从操作系统的角度来看，使用资源管理解决方案来管理 CPU 时间在应用程序之间的共享可能比绑定 mysqld 进程更可取。例如，可以在其他关键进程未运行时，将 90% 的虚拟 CPU 时间分配给某个应用程序，并在其他关键进程运行时将该值缩减到 40%。

  在某些情况下，最佳的 `innodb_thread_concurrency` 设置可能小于虚拟 CPU 的数量。

  过高的 `innodb_thread_concurrency` 值可能会导致由于系统内部和资源的争用增加而引起的性能回退。

  定期监控和分析系统。工作负载、用户数量或计算环境的变化可能需要调整 `innodb_thread_concurrency` 设置。

  值为 `0` 会禁用 `SHOW ENGINE INNODB STATUS` 输出的 `ROW OPERATIONS` 部分中的 InnoDB 内部查询和队列中的查询计数器。

  有关相关信息，请参见 [第17.8.4节, “配置 InnoDB 的线程并发性”](#17.8.4-配置-innodb-的线程并发性)。
...
- innodb_thread_sleep_delay
  ...
  InnoDB 线程在加入 InnoDB 队列前的睡眠时间，以微秒为单位。默认值为 10000。值为 `0` 禁用睡眠。可以将 `innodb_adaptive_max_sleep_delay` 设置为允许的最大值，InnoDB 会根据当前线程调度活动自动调整 `innodb_thread_sleep_delay` 的增减。这种动态调整有助于线程调度机制在系统负载较轻或接近满负荷运行时平稳工作。

  有关更多信息，请参见 [第17.8.4节, “配置 InnoDB 的线程并发性”](#17.8.4-配置-innodb-的线程并发性)。
...
- innodb_tmpdir
  ...
  用于定义在线 `ALTER TABLE` 操作期间创建的临时排序文件的备用目录，这些操作会重建表。

  在线 `ALTER TABLE` 操作在重建表时还会在与原始表相同的目录中创建一个中间表文件。`innodb_tmpdir` 选项不适用于中间表文件。

  有效值为 MySQL 数据目录路径以外的任何目录路径。如果值为 NULL（默认值），则临时文件将在 MySQL 临时目录（Unix 系统上的 $TMPDIR、Windows 系统上的 %TEMP% 或由 `--tmpdir` 配置选项指定的目录）中创建。如果指定了目录，则仅在使用 SET 语句配置 `innodb_tmpdir` 时检查目录的存在性和权限。如果在目录字符串中提供了符号链接，则解析符号链接并存储为绝对路径。路径不应超过 512 字节。 如果 `innodb_tmpdir` 设置为无效目录，则在线 `ALTER TABLE` 操作会报告错误。`innodb_tmpdir` 覆盖 MySQL `tmpdir` 设置，但仅适用于在线 `ALTER TABLE` 操作。

  配置 `innodb_tmpdir` 需要 FILE 权限。

  引入 `innodb_tmpdir` 选项是为了帮助避免在 tmpfs 文件系统上的临时文件目录溢出。这种溢出可能是由于在线 `ALTER TABLE` 操作期间创建的大型临时排序文件而发生的，这些操作会重建表。

  在复制环境中，只有在所有服务器具有相同的操作系统环境时才考虑复制 `innodb_tmpdir` 设置。否则，在运行重建表的在线 `ALTER TABLE` 操作时，复制 `innodb_tmpdir` 设置可能会导致复制失败。如果服务器操作环境不同，建议在每台服务器上单独配置 `innodb_tmpdir`。

  有关更多信息，请参见 [第17.12.3节, “在线 DDL 空间需求”](#17.12.3-在线-ddl-空间需求)。有关在线 `ALTER TABLE` 操作的信息，请参见 [第17.12节, “InnoDB 和在线 DDL”](#17.12-innodb-和在线-ddl)。
...
- innodb_trx_purge_view_update_only_debug
  ...
  暂停删除标记记录的清除，同时允许更新清除视图。此选项人为地创建一个情况，即清除视图已更新但尚未执行清除。此选项仅在使用 `WITH_DEBUG` CMake 选项编译时可用。
...
- innodb_trx_rseg_n_slots_debug
  ...
  设置一个调试标志，将 `TRX_RSEG_N_SLOTS` 限制为 `trx_rsegf_undo_find_free` 函数查找撤销日志段空闲槽位的给定值。此选项仅在使用 `WITH_DEBUG` CMake 选项编译时可用。
...
- innodb_undo_directory
  ...
  指定 InnoDB 创建撤销表空间的路径。通常用于将撤销表空间放置在不同的存储设备上。

  没有默认值（为 NULL）。如果未定义 `innodb_undo_directory` 变量，则在数据目录中创建撤销表空间。

  MySQL 实例初始化时创建的默认撤销表空间（`innodb_undo_001` 和 `innodb_undo_002`）始终位于由 `innodb_undo_directory` 变量定义的目录中。

  使用 `CREATE UNDO TABLESPACE` 语法创建的撤销表空间，如果未指定其他路径，则创建在由 `innodb_undo_directory` 变量定义的目录中。

  有关更多信息，请参见 [第17.6.3.4节, “撤销表空间”](#17.6.3.4-撤销表空间)。
...
- innodb_undo_log_encrypt
  ...
  控制使用 InnoDB 静态数据加密功能的表的撤销日志数据的加密。仅适用于位于单独撤销表空间中的撤销日志。参见 [第17.6.3.4节, “撤销表空间”](#17.6.3.4-撤销表空间)。不支持系统表空间中撤销日志数据的加密。有关更多信息，请参见撤销日志加密。
...
- innodb_undo_log_truncate
  ...
  启用时，超过 `innodb_max_undo_log_size` 阈值的撤销表空间将被标记为截断。仅撤销表空间可以被截断。不支持截断位于系统表空间中的撤销日志。要进行截断，必须至少有两个撤销表空间。

  `innodb_purge_rseg_truncate_frequency` 变量可用于加速撤销表空间的截断。
  ...
  定义 InnoDB 使用的撤销表空间的数量。默认值和最小值为2。

  > **注意**  
  > 从 MySQL 8.0.14 开始，`innodb_undo_tablespaces` 变量已弃用，不再可配置。预计将在未来版本中删除。
  ...
  在支持 `fdatasync()` 系统调用的平台上，启用 `innodb_use_fdatasync` 变量允许使用 `fdatasync()` 而不是 `fsync()` 系统调用进行操作系统刷新。`fdatasync()` 调用不会刷新文件元数据的更改，除非需要用于后续的数据检索，从而提供潜在的性能优势。

  `innodb_flush_method` 设置的一个子集（如 `fsync`、`O_DSYNC` 和 `O_DIRECT`）使用 `fsync()` 系统调用。`innodb_use_fdatasync` 变量适用于使用这些设置时。
  ...
  指定是否使用异步 I/O 子系统。此变量在服务器运行时无法更改。通常不需要配置此选项，因为默认情况下已启用。

  此功能提高了严重 I/O 受限系统的可扩展性，这些系统在 `SHOW ENGINE INNODB STATUS` 输出中通常显示许多待处理的读/写操作。

  在具有大量 InnoDB I/O 线程的情况下，尤其是在同一台服务器上运行多个实例时，可能会超出 Linux 系统的容量限制。在这种情况下，您可能会收到以下错误：

  ```plaintext
  EAGAIN: The specified maxevents exceeds the user's limit of available events.
  ```

  通常可以通过将较高的限制写入 `/proc/sys/fs/aio-max-nr` 来解决此错误。

  然而，如果操作系统中的异步 I/O 子系统存在问题，导致 InnoDB 无法启动，则可以使用 `innodb_use_native_aio=0` 启动服务器。如果 InnoDB 检测到潜在问题（例如，`tmpdir` 位置、`tmpfs` 文件系统和不支持 `tmpfs` 上 AIO 的 Linux 内核的组合），此选项也可能在启动期间自动禁用。

  有关更多信息，请参见 [第17.8.6节, “在 Linux 上使用异步 I/O”](#17.8.6-在-linux-上使用异步-io)。
  ...
  控制表空间文件路径验证。启动时，InnoDB 验证已知表空间文件的路径与数据字典中存储的表空间文件路径，以防表空间文件已移至其他位置。`innodb_validate_tablespace_paths` 变量允许禁用表空间路径验证。此功能旨在适用于表空间文件不移动的环境。禁用路径验证可以提高具有大量表空间文件的系统的启动时间。

  > **警告**  
  > 在移动表空间文件后禁用表空间路径验证启动服务器可能导致未定义行为。

  有关更多信息，请参见 [第17.6.3.7节, “禁用表空间路径验证”](#17.6.3.7-禁用表空间路径验证)。

  ### innodb_version

  InnoDB 版本号。在 MySQL 8.0 中，InnoDB 的单独版本编号不适用，此值与服务器版本号相同。
  ...
  InnoDB 写操作的 I/O 线程数。默认值为 4。其读线程的对应变量是 `innodb_read_io_threads`。有关更多信息，请参见 [第17.8.5节, “配置后台 InnoDB I/O 线程数”](#17.8.5-配置后台-innodb-io-线程数)。有关一般 I/O 调优建议，请参见 [第10.5.8节, “优化 InnoDB 磁盘 I/O”](#10.5.8-优化-innodb-磁盘-io)。

  > **注意**  
  > 在 Linux 系统上，运行多个 MySQL 服务器（通常超过 12 个）时，如果 `innodb_read_io_threads`、`innodb_write_io_threads` 和 Linux `aio-max-nr` 设置为默认值，可能会超出系统限制。理想情况下，应增加 `aio-max-nr` 设置；作为一种变通方法，可以减少一个或两个 MySQL 变量的设置。

  还要考虑 `sync_binlog` 的值，它控制二进制日志同步到磁盘的同步。

  有关一般 I/O 调优建议，请参见 [第10.5.8节, “优化 InnoDB 磁盘 I/O”](#10.5.8-优化-innodb-磁盘-io)。
