...
1. This is true in either case whether not `pt` has an explicit table-level `MAX_ROWS` or `MIN_ROWS` option in effect.
2. The `AVG_ROW_LENGTH` cannot differ between the two tables `pt` and `nt`.

1. 表 `pt` 没有使用 `DATA DIRECTORY` 选项的分区。此限制在 MySQL 8.0.14 及更高版本的 InnoDB 表中已取消。



1. `INDEX DIRECTORY` 在表和要与其交换的分区之间不能不同。
2. 两个表中都不能使用任何 `TABLESPACE` 选项。
