...
表级 `DATA DIRECTORY` 和 `INDEX DIRECTORY` 选项被忽略（参见 Bug #32091）。可以对 InnoDB 表的各个分区或子分区使用这些选项。从 MySQL 8.0.21 开始，在 `DATA DIRECTORY` 子句中指定的目录必须为 InnoDB 所知。有关更多信息，请参见 [使用 DATA DIRECTORY 子句](https://dev.mysql.com/doc/refman/8.0/en/innodb-data-directory.html)。

### 修复和重建分区表
支持对分区表执行 `CHECK TABLE`、`OPTIMIZE TABLE`、`ANALYZE TABLE` 和 `REPAIR TABLE` 语句。
...
### 分区和子分区的文件名分隔符
表分区和子分区文件名包含生成的分隔符（如 `#P#` 和 `#SP#`）。这些分隔符的字母大小写可能会有所不同，且不应依赖。